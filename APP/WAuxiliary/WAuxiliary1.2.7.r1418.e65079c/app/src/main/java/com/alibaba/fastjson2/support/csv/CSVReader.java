...
public abstract class CSVReader<T> extends StreamReader<T> implements Closeable {
    ...
    public Object error(int i, Exception exc) {
        ...
        String strM2705Ujhhgtgfeyxiexzf = AbstractC1225feyxiexzfUjhhgtg.m2705Ujhhgtgfeyxiexzf(new StringBuilder("read csv error, line "), this.rowCount, ", column ");
        List<String> list = this.columns;
        if (list != null && i < list.size()) {
            str = this.columns.get(i);
        }
        if (str == null || str.isEmpty()) {
            strM2703Ujhhgtgfeyxiexzf = strM2705Ujhhgtgfeyxiexzf + i;
        } else {
            strM2703Ujhhgtgfeyxiexzf = AbstractC1225feyxiexzfUjhhgtg.m2703Ujhhgtgfeyxiexzf(strM2705Ujhhgtgfeyxiexzf, str);
        }
        throw new JSONException(strM2703Ujhhgtgfeyxiexzf, exc);
    }
    ...
    public List<String> readHeader() {
        ...
        IntStream.range(0, strArr.length).forEach(new IntConsumer() { // from class: 上海高中ᛱ谢子非ᛱ能不能ᛲᛴᛱUjhhgtgᛱᛳ要点脸ᛱfeyxiexzfᛱ
            @Override // java.util.function.IntConsumer
            public final void accept(int i2) {
                this.f8464Ujhhgtgfeyxiexzf.lambda$readHeader$0(strArr, i2);
            }
        });
        if (this.rowCount == 1) {
