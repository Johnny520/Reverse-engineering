...
final class ObjectReaderImplHijrahDate extends ObjectReaderPrimitive<HijrahDate> {
    static final ObjectReaderImplHijrahDate INSTANCE = new ObjectReaderImplHijrahDate();

    public ObjectReaderImplHijrahDate() {
        super(HijrahDate.class);
    }
    ...
    private static ChronoLocalDate parseChronoLocalDate(String str, Chronology chronology) {
        ...
        if (!chronology.getId().equals(strSubstring)) {
            throw new DateTimeException(AbstractC1225feyxiexzfUjhhgtg.m2711Ujhhgtgfeyxiexzf("Invalid chronology: ", strSubstring));
        }
        String strSubstring2 = str.substring(i, iIndexOf2);
        Era eraFindEra = findEra(chronology, strSubstring2);
        if (eraFindEra == null) {
            throw new DateTimeException(AbstractC1225feyxiexzfUjhhgtg.m2711Ujhhgtgfeyxiexzf("Invalid era: ", strSubstring2));
        }
    ...
    public HijrahDate readJSONBObject(JSONReader jSONReader, Type type, Object obj, long j) {
        String string = jSONReader.readString();
        if (string == null || string.isEmpty()) {
            return null;
        }
        return (HijrahDate) parseChronoLocalDate(string, HijrahChronology.INSTANCE);
    }

    @Override // com.alibaba.fastjson2.reader.ObjectReader
    public HijrahDate readObject(JSONReader jSONReader, Type type, Object obj, long j) {
        String string = jSONReader.readString();
        if (string == null) {
            return null;
        }
