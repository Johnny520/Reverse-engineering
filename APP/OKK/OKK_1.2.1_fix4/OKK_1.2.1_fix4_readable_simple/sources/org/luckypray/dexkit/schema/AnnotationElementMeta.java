package org.luckypray.dexkit.schema;

import com.google.flatbuffers.AbstractC0735d;
import com.google.flatbuffers.C0733b;
import java.nio.ByteBuffer;
import java.nio.ByteOrder;
import p031Q0.AbstractC0304d;
import p031Q0.AbstractC0307g;
import p034S.AbstractC0324d;

/* JADX INFO: renamed from: org.luckypray.dexkit.schema.-AnnotationElementMeta */
/* JADX INFO: loaded from: classes.dex */
public final class AnnotationElementMeta extends AbstractC0735d {
    public static final Companion Companion = null;

    /* JADX INFO: renamed from: org.luckypray.dexkit.schema.-AnnotationElementMeta$Companion */
    public static final class Companion {
        private Companion() {
        }

        public final void addName(C0733b r2, int r3) {
            AbstractC0307g.m703e(r2, "builder");
            r2.m1925h(0, r3);
        }

        public final void addValue(C0733b r2, int r3) {
            AbstractC0307g.m703e(r2, "builder");
            r2.m1925h(1, r3);
        }

        public final int createAnnotationElementMeta(C0733b r2, int r3, int r4) {
            AbstractC0307g.m703e(r2, "builder");
            r2.m1936s(2);
            addValue(r2, r4);
            addName(r2, r3);
            return endAnnotationElementMeta(r2);
        }

        public final int endAnnotationElementMeta(C0733b r2) {
            AbstractC0307g.m703e(r2, "builder");
            return r2.m1929l();
        }

        public final AnnotationElementMeta getRootAsAnnotationElementMeta(ByteBuffer r2) {
            AbstractC0307g.m703e(r2, "_bb");
            return getRootAsAnnotationElementMeta(r2, new AnnotationElementMeta());
        }

        public final void startAnnotationElementMeta(C0733b r2) {
            AbstractC0307g.m703e(r2, "builder");
            r2.m1936s(2);
        }

        public final void validateVersion() {
        }

        public /* synthetic */ Companion(AbstractC0304d r1) {
            this();
        }

        public final AnnotationElementMeta getRootAsAnnotationElementMeta(ByteBuffer r3, AnnotationElementMeta r4) {
            AbstractC0307g.m703e(r3, "_bb");
            AbstractC0307g.m703e(r4, "obj");
            int r02 = AbstractC0324d.m719b(r3, ByteOrder.LITTLE_ENDIAN);
            return r4.__assign(r3.position() + r02, r3);
        }
    }

    static {
        Companion = new Companion(null);
    }

    public AnnotationElementMeta() {
    }

    public final AnnotationElementMeta __assign(int r2, ByteBuffer r3) {
        AbstractC0307g.m703e(r3, "_bb");
        __init(r2, r3);
        return this;
    }

    public final void __init(int r2, ByteBuffer r3) {
        AbstractC0307g.m703e(r3, "_bb");
        __reset(r2, r3);
    }

    public final String getName() {
        int r02 = __offset(4);
        if (r02 != 0) goto L7;
        return null;
    L7:
        return __string(r02 + this.bb_pos);
    }

    public final ByteBuffer getNameAsByteBuffer() {
        ByteBuffer r02 = __vector_as_bytebuffer(4, 1);
        AbstractC0307g.m702d(r02, "__vector_as_bytebuffer(4, 1)");
        return r02;
    }

    public final AnnotationEncodeValueMeta getValue() {
        return value(new AnnotationEncodeValueMeta());
    }

    public final ByteBuffer nameInByteBuffer(ByteBuffer r3) {
        AbstractC0307g.m703e(r3, "_bb");
        ByteBuffer r32 = __vector_in_bytebuffer(r3, 4, 1);
        AbstractC0307g.m702d(r32, "__vector_in_bytebuffer(_bb, 4, 1)");
        return r32;
    }

    public final AnnotationEncodeValueMeta value(AnnotationEncodeValueMeta r4) {
        AbstractC0307g.m703e(r4, "obj");
        int r02 = __offset(6);
        if (r02 == 0) goto L5;
        int r03 = __indirect(r02 + this.bb_pos);
        ByteBuffer r1 = this.f2573bb;
        AbstractC0307g.m702d(r1, "bb");
        return r4.__assign(r03, r1);
    L5:
        return null;
    }
}
