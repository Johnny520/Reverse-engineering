.class public final Ltc;
.super Lcom/mr/elaris/xposedcompat/XC_MethodHook;
.source "r8-map-id-cb25a2bafef54619fff8cdac9d36ff56d89ce0d5459e21e33664152f938f1f39"


# instance fields
.field public final synthetic a:I

.field public final synthetic b:Ljava/lang/reflect/Method;

.field public final synthetic c:I


# direct methods
.method public constructor <init>(ILjava/lang/reflect/Method;I)V
    .locals 0

    .line 1
    iput p1, p0, Ltc;->a:I

    .line 2
    .line 3
    iput-object p2, p0, Ltc;->b:Ljava/lang/reflect/Method;

    .line 4
    .line 5
    iput p3, p0, Ltc;->c:I

    .line 6
    .line 7
    const/16 p1, 0x78

    .line 8
    .line 9
    invoke-direct {p0, p1}, Lcom/mr/elaris/xposedcompat/XC_MethodHook;-><init>(I)V

    .line 10
    .line 11
    .line 12
    return-void
.end method


# virtual methods
.method public final afterHookedMethod(Lcom/mr/elaris/xposedcompat/XC_MethodHook$MethodHookParam;)V
    .locals 0

    .line 1
    invoke-static {}, Lvc;->b()V

    .line 2
    .line 3
    .line 4
    return-void
.end method

.method public final beforeHookedMethod(Lcom/mr/elaris/xposedcompat/XC_MethodHook$MethodHookParam;)V
    .locals 4

    .line 1
    iget-object v0, p1, Lcom/mr/elaris/xposedcompat/XC_MethodHook$MethodHookParam;->args:[Ljava/lang/Object;

    .line 2
    .line 3
    const/4 v1, 0x0

    .line 4
    if-eqz v0, :cond_0

    .line 5
    .line 6
    array-length v2, v0

    .line 7
    iget v3, p0, Ltc;->a:I

    .line 8
    .line 9
    if-le v2, v3, :cond_0

    .line 10
    .line 11
    aget-object v0, v0, v3

    .line 12
    .line 13
    goto :goto_0

    .line 14
    :cond_0
    move-object v0, v1

    .line 15
    :goto_0
    iget-object v2, p0, Ltc;->b:Ljava/lang/reflect/Method;

    .line 16
    .line 17
    const/4 v3, 0x0

    .line 18
    invoke-static {v0, v2, v3}, Lvc;->c(Ljava/lang/Object;Ljava/lang/reflect/Method;Z)V

    .line 19
    .line 20
    .line 21
    iget p0, p0, Ltc;->c:I

    .line 22
    .line 23
    if-ltz p0, :cond_4

    .line 24
    .line 25
    sget-object v2, Lvc;->d:Ljava/lang/ThreadLocal;

    .line 26
    .line 27
    invoke-virtual {v2}, Ljava/lang/ThreadLocal;->get()Ljava/lang/Object;

    .line 28
    .line 29
    .line 30
    move-result-object v2

    .line 31
    check-cast v2, Ljava/util/ArrayDeque;

    .line 32
    .line 33
    if-eqz v2, :cond_2

    .line 34
    .line 35
    invoke-virtual {v2}, Ljava/util/ArrayDeque;->isEmpty()Z

    .line 36
    .line 37
    .line 38
    move-result v3

    .line 39
    if-eqz v3, :cond_1

    .line 40
    .line 41
    goto :goto_1

    .line 42
    :cond_1
    invoke-virtual {v2}, Ljava/util/ArrayDeque;->peek()Ljava/lang/Object;

    .line 43
    .line 44
    .line 45
    move-result-object v2

    .line 46
    goto :goto_2

    .line 47
    :cond_2
    :goto_1
    move-object v2, v1

    .line 48
    :goto_2
    sget-object v3, Lvc;->b:Ljava/lang/Object;

    .line 49
    .line 50
    if-ne v2, v3, :cond_3

    .line 51
    .line 52
    goto :goto_3

    .line 53
    :cond_3
    move-object v1, v2

    .line 54
    :goto_3
    if-ne v1, v0, :cond_4

    .line 55
    .line 56
    iget-object p1, p1, Lcom/mr/elaris/xposedcompat/XC_MethodHook$MethodHookParam;->args:[Ljava/lang/Object;

    .line 57
    .line 58
    array-length v0, p1

    .line 59
    if-le v0, p0, :cond_4

    .line 60
    .line 61
    sget-object v0, Ljava/lang/Boolean;->FALSE:Ljava/lang/Boolean;

    .line 62
    .line 63
    aput-object v0, p1, p0

    .line 64
    .line 65
    :cond_4
    return-void
.end method
