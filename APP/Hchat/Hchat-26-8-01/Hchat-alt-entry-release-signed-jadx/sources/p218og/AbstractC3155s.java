package p218og;

import okhttp3.internal.http2.Settings;

/* JADX INFO: renamed from: og.s */
/* JADX INFO: compiled from: r8-map-id-410253dec230a6d44f261a3a84880494196f082735db3c71240f387861686496 */
/* JADX INFO: loaded from: classes.dex */
public abstract class AbstractC3155s extends AbstractC3154r {
    /* JADX DEBUG: Don't trust debug lines info. Lines numbers was adjusted: min line is 1 */
    /* JADX WARN: Removed duplicated region for block: B:106:0x0121  */
    /* JADX WARN: Removed duplicated region for block: B:69:0x00c5  */
    /* JADX INFO: renamed from: T */
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public static final boolean m6730T(String str) {
        char c10;
        boolean z9;
        boolean z10;
        int i9;
        boolean z11;
        boolean z12;
        boolean z13 = true;
        int length = str.length() - 1;
        int i10 = 0;
        while (true) {
            c10 = ' ';
            if (i10 > length || str.charAt(i10) > ' ') {
                break;
            }
            i10++;
        }
        if (i10 > length) {
            return false;
        }
        while (length > i10 && str.charAt(length) <= ' ') {
            length--;
        }
        if (str.charAt(i10) == '+' || str.charAt(i10) == '-') {
            i10++;
        }
        if (i10 > length) {
            return false;
        }
        if (str.charAt(i10) != '0') {
            z9 = true;
            z10 = false;
        } else {
            int i11 = i10 + 1;
            if (i11 > length) {
                return true;
            }
            if ((str.charAt(i11) | ' ') == 120) {
                int i12 = i10 + 2;
                int i13 = i12;
                while (true) {
                    if (i13 > length) {
                        z9 = z13;
                        break;
                    }
                    z9 = z13;
                    if (((str.charAt(i13) - '0') & Settings.DEFAULT_INITIAL_WINDOW_SIZE) >= 10 && (((r15 | ' ') - 97) & Settings.DEFAULT_INITIAL_WINDOW_SIZE) >= 6) {
                        break;
                    }
                    i13++;
                    z13 = z9;
                }
                boolean z14 = i12 != i13 ? z9 : false;
                if (i13 <= length) {
                    if (str.charAt(i13) == '.') {
                        int i14 = i13 + 1;
                        int i15 = i14;
                        while (i15 <= length) {
                            char c11 = c10;
                            if (((str.charAt(i15) - '0') & Settings.DEFAULT_INITIAL_WINDOW_SIZE) >= 10 && (((r15 | ' ') - 97) & Settings.DEFAULT_INITIAL_WINDOW_SIZE) >= 6) {
                                break;
                            }
                            i15++;
                            c10 = c11;
                        }
                        z12 = i14 != i15 ? z9 : false;
                        i13 = i15;
                    } else {
                        z12 = false;
                    }
                    if (z14 || z12) {
                        i10 = i13;
                    }
                    if (i10 != -1 || i10 > length) {
                        return false;
                    }
                    z10 = z9;
                }
                i10 = -1;
                if (i10 != -1) {
                }
                return false;
            }
        }
        if (!z10) {
            int i16 = i10;
            while (i16 <= length && ((str.charAt(i16) - '0') & Settings.DEFAULT_INITIAL_WINDOW_SIZE) < 10) {
                i16++;
            }
            boolean z15 = i10 != i16 ? z9 : false;
            if (i16 > length) {
                i10 = i16;
            } else {
                if (str.charAt(i16) == '.') {
                    int i17 = i16 + 1;
                    i9 = i17;
                    while (i9 <= length && ((str.charAt(i9) - '0') & Settings.DEFAULT_INITIAL_WINDOW_SIZE) < 10) {
                        i9++;
                    }
                    if (i17 != i9) {
                        z11 = z9;
                    }
                    if (!z15 || z11) {
                        i10 = i9;
                    } else {
                        String str2 = length == i9 + 2 ? "NaN" : length == i9 + 7 ? "Infinity" : null;
                        i10 = (str2 != null && AbstractC3149m.m6716o0(str, str2, i9, false) == i9) ? length + 1 : -1;
                    }
                } else {
                    i9 = i16;
                }
                z11 = false;
                if (z15) {
                    i10 = i9;
                }
            }
            if (i10 == -1) {
                return false;
            }
            if (i10 > length) {
                return z9;
            }
        }
        int i18 = i10 + 1;
        int iCharAt = str.charAt(i10) | ' ';
        if (iCharAt != (z10 ? 112 : 101)) {
            if (z10 || (!(iCharAt == 102 || iCharAt == 100) || i18 <= length)) {
                return false;
            }
            return z9;
        }
        if (i18 > length) {
            return false;
        }
        if ((str.charAt(i18) == '+' || str.charAt(i18) == '-') && (i18 = i10 + 2) > length) {
            return false;
        }
        while (i18 <= length && ((str.charAt(i18) - '0') & Settings.DEFAULT_INITIAL_WINDOW_SIZE) < 10) {
            i18++;
        }
        if (i18 > length) {
            return z9;
        }
        if (i18 != length) {
            return false;
        }
        int iCharAt2 = str.charAt(i18) | ' ';
        if (iCharAt2 == 102 || iCharAt2 == 100) {
            return z9;
        }
        return false;
    }

    /* JADX DEBUG: Don't trust debug lines info. Lines numbers was adjusted: min line is 1 */
    /* JADX INFO: renamed from: U */
    public static Double m6731U(String str) {
        str.getClass();
        try {
            if (m6730T(str)) {
                return Double.valueOf(Double.parseDouble(str));
            }
        } catch (NumberFormatException unused) {
        }
        return null;
    }
}
