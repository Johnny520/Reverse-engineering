...
public abstract class MethodConstant extends StackManipulation.AbstractBase {
    ...
    public static class CachedConstructor implements StackManipulation {
        private static final TypeDescription CONSTRUCTOR_TYPE = TypeDescription.ForLoadedType.m5238of(Constructor.class);
    ...
    public static class CachedMethod implements StackManipulation {
        private static final TypeDescription METHOD_TYPE = TypeDescription.ForLoadedType.m5238of(Method.class);
    ...
    public static class PrivilegedLookup implements StackManipulation, CanCache {
        ...
        public StackManipulation.Size apply(MethodVisitor methodVisitor, Implementation.Context context) {
            ...
            TypeDescription typeDescriptionRegister = context.register(PrivilegedMemberLookupAction.m5361of(this.methodDescription));
            return new StackManipulation.Compound(TypeCreation.m5375of(typeDescriptionRegister), Duplication.SINGLE, ClassConstant.m5380of(this.methodDescription.getDeclaringType()), this.methodName, ArrayFactory.forType(TypeDescription.Generic.OfNonGenericType.ForLoadedType.m5245of(Class.class)).withValues(MethodConstant.typeConstantsFor(this.methodDescription.getParameters().asTypeList().asErasures())), MethodInvocation.invoke((MethodDescription.InDefinedShape) typeDescriptionRegister.getDeclaredMethods().filter(ElementMatchers.isConstructor()).getOnly()), MethodInvocation.invoke(inDefinedShape), TypeCasting.m5378to(TypeDescription.ForLoadedType.m5238of(this.methodDescription.isConstructor() ? Constructor.class : Method.class))).apply(methodVisitor, context);
        }
    ...
    public static CanCache m5382of(MethodDescription.InDefinedShape inDefinedShape) {
    ...
    public static CanCache ofPrivileged(MethodDescription.InDefinedShape inDefinedShape) {
        return DO_PRIVILEGED == null ? m5382of(inDefinedShape) : inDefinedShape.isTypeInitializer() ? CanCacheIllegal.INSTANCE : inDefinedShape.isConstructor() ? new ForConstructor(inDefinedShape).withPrivilegedLookup() : new ForMethod(inDefinedShape).withPrivilegedLookup();
    }
    ...
    public static List<StackManipulation> typeConstantsFor(List<TypeDescription> list) {
        ...
        while (it.hasNext()) {
            arrayList.add(ClassConstant.m5380of(it.next()));
        }
        return arrayList;
    }
    ...
    @Override // net.bytebuddy.implementation.bytecode.StackManipulation
    public StackManipulation.Size apply(MethodVisitor methodVisitor, Implementation.Context context) {
        return new StackManipulation.Compound(ClassConstant.m5380of(this.methodDescription.getDeclaringType()), methodName(), ArrayFactory.forType(TypeDescription.Generic.OfNonGenericType.ForLoadedType.m5245of(Class.class)).withValues(typeConstantsFor(this.methodDescription.getParameters().asTypeList().asErasures())), MethodInvocation.invoke(accessorMethod())).apply(methodVisitor, context);
    }
