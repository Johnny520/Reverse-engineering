...
public @interface AllArguments {
    ...
    public enum Binder implements TargetMethodAnnotationDrivenBinder.ParameterBinder<AllArguments> {
        ...
        public MethodDelegationBinder.ParameterBinding<?> bind(AnnotationDescription.Loadable<AllArguments> loadable, MethodDescription methodDescription, ParameterDescription parameterDescription, Implementation.Target target, Assigner assigner, Assigner.Typing typing) {
            ...
            for (TypeDescription.Generic generic : i != 0 ? CompoundList.m5412of(target.getInstrumentedType().asGenericType(), methodDescription.getParameters().asTypeList()) : methodDescription.getParameters().asTypeList()) {
                StackManipulation.Compound compound = new StackManipulation.Compound(MethodVariableAccess.m5387of(generic).loadFrom(size), assigner.assign(generic, componentType, typing));
                if (compound.isValid()) {
