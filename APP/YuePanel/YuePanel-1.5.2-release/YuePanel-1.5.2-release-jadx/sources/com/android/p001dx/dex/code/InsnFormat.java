...
import com.android.p001dx.rop.cst.CstLiteral64;
...
public abstract class InsnFormat {
    ...
    public static String branchComment(DalvInsn dalvInsn) {
        ...
        return targetOffset == ((short) targetOffset) ? Hex.m5031s2(targetOffset) : Hex.m5032s4(targetOffset);
    }

    public static String branchString(DalvInsn dalvInsn) {
        int targetAddress = ((TargetInsn) dalvInsn).getTargetAddress();
        return targetAddress == ((char) targetAddress) ? Hex.m5035u2(targetAddress) : Hex.m5037u4(targetAddress);
    }
    ...
    public static String literalBitsComment(CstLiteralBits cstLiteralBits, int i) {
        ...
        if (i == 4) {
            ...
            sb.append(Hex.m5034u1((int) longBits));
        } else if (i == 16) {
            sb.append(Hex.m5035u2((int) longBits));
        } else if (i == 32) {
            sb.append(Hex.m5037u4((int) longBits));
        } else {
            if (i != 64) {
                throw new RuntimeException("shouldn't happen");
            }
            sb.append(Hex.m5038u8(longBits));
        }
        return sb.toString();
    }
