...
public class LoadPathCache {
    ...
    private final C3394<MultiClassKey, LoadPath<?, ?, ?>> cache = new C3394<>();
    ...
    public boolean isEmptyLoadPath(@InterfaceC6490 LoadPath<?, ?, ?> loadPath) {
        return NO_PATHS_SIGNAL.equals(loadPath);
    }

    public void put(Class<?> cls, Class<?> cls2, Class<?> cls3, @InterfaceC6490 LoadPath<?, ?, ?> loadPath) {
        synchronized (this.cache) {
            C3394<MultiClassKey, LoadPath<?, ?, ?>> c3394 = this.cache;
            MultiClassKey multiClassKey = new MultiClassKey(cls, cls2, cls3);
            if (loadPath == null) {
                loadPath = NO_PATHS_SIGNAL;
            }
            c3394.put(multiClassKey, loadPath);
        }
    }
}
