...
public class DotDumper implements ParseObserver {
    ...
    @Override // com.android.p001dx.p002cf.iface.ParseObserver
    ...
    public void endParsingMember(ByteArray byteArray, int i, String str, String str2, Member member) {
        if ((member instanceof Method) && shouldDumpMethod(str)) {
            ...
            printStream.println("\tfirst -> n" + Hex.m5035u2(ropMethodConvert.getFirstLabel()) + ";");
            ...
            while (i3 < size) {
                ...
                if (successors.size() == 0) {
                    System.out.println("\tn" + Hex.m5035u2(label) + " -> returns;");
                } else if (successors.size() == 1) {
                    System.out.println("\tn" + Hex.m5035u2(label) + " -> n" + Hex.m5035u2(successors.get(i2)) + ";");
                } else {
                    System.out.print("\tn" + Hex.m5035u2(label) + " -> {");
                    for (int i4 = i2; i4 < successors.size(); i4++) {
                        int i5 = successors.get(i4);
                        if (i5 != basicBlock.getPrimarySuccessor()) {
                            System.out.print(" n" + Hex.m5035u2(i5) + " ");
                        }
                    }
                    PrintStream printStream2 = System.out;
                    printStream2.println("};");
                    printStream2.println("\tn" + Hex.m5035u2(label) + " -> n" + Hex.m5035u2(basicBlock.getPrimarySuccessor()) + " [label=\"primary\"];");
                }
                i3++;
                i2 = 0;
            }
