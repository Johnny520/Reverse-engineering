...
public class CachedReturnPlugin extends Plugin.ForElementMatcher implements Plugin.Factory {
    ...
    private static final MethodDescription.InDefinedShape ENHANCE_VALUE = (MethodDescription.InDefinedShape) TypeDescription.ForLoadedType.m5238of(Enhance.class).getDeclaredMethods().filter(ElementMatchers.named("value")).getOnly();
    ...
    public DynamicType.Builder<?> apply(DynamicType.Builder<?> builder, TypeDescription typeDescription, ClassFileLocator classFileLocator) {
        for (MethodDescription.InDefinedShape inDefinedShape : typeDescription.getDeclaredMethods().filter(ElementMatchers.not(ElementMatchers.isBridge()).and(ElementMatchers.isAnnotatedWith((Class<? extends Annotation>) Enhance.class)))) {
            ...
            builder = builder.defineField(str, inDefinedShape.getReturnType().asErasure(), inDefinedShape.isStatic() ? Ownership.STATIC : Ownership.MEMBER, inDefinedShape.isStatic() ? FieldPersistence.PLAIN : FieldPersistence.TRANSIENT, Visibility.PRIVATE, SyntheticState.SYNTHETIC).visit(Advice.withCustomMapping().bind(CacheField.class, (Advice.OffsetMapping) new CacheFieldOffsetMapping(str)).m5174to(this.adviceByType.get(inDefinedShape.getReturnType().isPrimitive() ? inDefinedShape.getReturnType().asErasure() : TypeDescription.ForLoadedType.m5238of(Object.class)), this.classFileLocator).m5147on(ElementMatchers.m5397is(inDefinedShape)));
        }
        return builder;
    }
    ...
    public CachedReturnPlugin(boolean z) {
        ...
        ClassFileLocator classFileLocatorM5258of = ClassFileLocator.ForClassLoader.m5258of(CachedReturnPlugin.class.getClassLoader());
        this.classFileLocator = classFileLocatorM5258of;
        TypePool typePoolM5405of = TypePool.Default.m5405of(classFileLocatorM5258of);
        ...
        for (int i = 0; i < 9; i++) {
            Class cls = clsArr[i];
            this.adviceByType.put(TypeDescription.ForLoadedType.m5238of(cls), typePoolM5405of.describe(CachedReturnPlugin.class.getName() + ADVICE_INFIX + cls.getSimpleName()).resolve());
        }
    }
}
