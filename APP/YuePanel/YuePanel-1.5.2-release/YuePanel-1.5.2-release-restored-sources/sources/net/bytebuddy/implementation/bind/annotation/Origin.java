...
public @interface Origin {
    ...
    public enum Binder implements TargetMethodAnnotationDrivenBinder.ParameterBinder<Origin> {
        ...
        static {
            MethodList<MethodDescription.InDefinedShape> declaredMethods = TypeDescription.ForLoadedType.m5238of(Origin.class).getDeclaredMethods();
        ...
        private static StackManipulation methodConstant(AnnotationDescription.Loadable<Origin> loadable, MethodDescription.InDefinedShape inDefinedShape) {
            MethodConstant.CanCache canCacheOfPrivileged = ((Boolean) loadable.getValue(PRIVILEGED).resolve(Boolean.class)).booleanValue() ? MethodConstant.ofPrivileged(inDefinedShape) : MethodConstant.m5382of(inDefinedShape);
        ...
        public MethodDelegationBinder.ParameterBinding<?> bind(AnnotationDescription.Loadable<Origin> loadable, MethodDescription methodDescription, ParameterDescription parameterDescription, Implementation.Target target, Assigner assigner, Assigner.Typing typing) {
            ...
            if (typeDescriptionAsErasure.represents(Class.class)) {
                return new MethodDelegationBinder.ParameterBinding.Anonymous(ClassConstant.m5380of(target.getOriginType().asErasure()));
            }
            ...
            if (typeDescriptionAsErasure.equals(JavaType.METHOD_HANDLE.getTypeStub())) {
                return new MethodDelegationBinder.ParameterBinding.Anonymous(JavaConstant.MethodHandle.m5418of(methodDescription.asDefined()).toStackManipulation());
            }
            if (typeDescriptionAsErasure.equals(JavaType.METHOD_TYPE.getTypeStub())) {
                return new MethodDelegationBinder.ParameterBinding.Anonymous(JavaConstant.MethodType.m5424of(methodDescription.asDefined()).toStackManipulation());
            }
