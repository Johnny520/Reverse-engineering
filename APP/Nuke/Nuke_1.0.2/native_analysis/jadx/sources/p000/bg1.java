...
public abstract class bg1 extends x73 {
    ...
    public final void mo399k() {
        if (mo398c().size() <= 65536) {
            Iterator it = mo398c().iterator();
            int i = 0;
            while (it.hasNext()) {
                ((ag1) it.next()).m2417g(i);
                i++;
            }
            return;
        }
        TreeMap treeMap = new TreeMap();
        Iterator it2 = mo398c().iterator();
        while (it2.hasNext()) {
            String str = ((ag1) it2.next()).f3733i.m4328g().f8714h;
        ...
        try {
            formatter.format("Too many %1$s references to fit in one dex file: %2$d; max is %3$d.%nYou may try using multi-dex. If multi-dex is enabled then the list of classes for the main dex list is too large.%nReferences by package:", this instanceof xg1 ? "method" : "field", Integer.valueOf(mo398c().size()), 65536);
