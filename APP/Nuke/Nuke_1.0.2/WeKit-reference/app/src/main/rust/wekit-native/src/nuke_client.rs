...
pub const NUKE_NATIVE_GENERATION: &str = "2026071502";
pub const DEFAULT_NUKE_BUILD_TIME_MILLIS: u64 = 1_785_243_782_422;
pub const DEFAULT_CLIENT_REPORT_MESSAGE: &str =
    "Nuke onLogin: /, module=1.0.2(release), buildTime=1785243782422";
...
///
/// Field order follows the 1.0.2 loader's `nt` data class. Its default Gson
/// instance uses `serializeNulls:false`, so absent Android observations are
/// omitted instead of being emitted as JSON nulls.
...
pub fn native_on_login_message(nickname: &str, alias: &str) -> String {
    format!(
        "Nuke onLogin: {nickname}/{alias}, module=1.0.2(release), buildTime={DEFAULT_NUKE_BUILD_TIME_MILLIS}"
    )
}
...
pub fn fixed_native_wechat_report() -> ClientReportRequest {
    let extra = json!({
        ...
        "dirtySepolicy": {
            ...
            "markers": {
                "system_server_execmem": {
                    "hit": false,
                    "available": true,
                    "method": "android.os.SELinux",
                },
                "aosp_su_transition": {
                    "hit": false,
                    "available": true,
                ...
                "adbroot_context": {
                    "hit": false,
                    "available": true,
                    "method": "selinuxfs-context",
                },
                "magisk_context": {
                    "hit": false,
                    "available": true,
                    "method": "selinuxfs-context+selinuxfs-context+android.os.SELinux+android.os.SELinux",
                },
                "kernelsu_context": {
                    "hit": true,
                    "available": true,
                    "method": "selinuxfs-context+selinuxfs-context+android.os.SELinux",
                },
                "lsposed_context": {
                    "hit": true,
                    "available": true,
                    "method": "selinuxfs-context+android.os.SELinux",
                },
                "xposed_context": {
                    "hit": true,
                    "available": true,
            ...
            "detected": ["kernelsu_context", "lsposed_context", "xposed_context"],
        },
    });
...
/// Builds the legacy native-oracle signature with explicit runtime
/// observations. This is retained for deterministic vectors; generation
/// 2026071502 additionally binds cloud signing to bridge/module attestation.
...
/// The native dispatcher signs this request with the same eight-line canonical
...
fn post_signed_request(
    ...
    let response = client
        ...
        .header("X-Native-Generation", NUKE_NATIVE_GENERATION)
...
mod tests {
    ...
    fn fixed_report_matches_successful_experiments_message_and_sepolicy() {
        ...
        assert_eq!(environment["androidVersion"], "16");
        assert_eq!(environment["androidSdkInt"], 36);
        ...
        assert_eq!(environment["appVersion"], "1.0.2");
        assert_eq!(environment["appBuild"], "234");
        ...
        assert!(environment.get("xposedVersion").is_none());
        assert_eq!(environment["xposedInjectionMode"], "unknown");
        assert!(environment.get("isRooted").is_none());
        assert!(environment.get("isEmulator").is_none());
        assert!(environment.get("installerPackage").is_none());
        assert_eq!(environment["locale"], "zh-CN");
        assert_eq!(environment["timezone"], "Asia/Shanghai");
        assert!(environment.get("networkType").is_none());
        ...
        assert_eq!(dirty_sepolicy["markers"]["magisk_context"]["hit"], false);
        assert_eq!(dirty_sepolicy["markers"]["kernelsu_context"]["hit"], true);
        assert_eq!(dirty_sepolicy["markers"]["lsposed_context"]["hit"], true);
        assert_eq!(dirty_sepolicy["markers"]["xposed_context"]["hit"], true);
        assert_eq!(dirty_sepolicy["markers"]["zygisk_next_rule"]["hit"], false);
        assert_eq!(
            dirty_sepolicy["detected"],
            json!(["kernelsu_context", "lsposed_context", "xposed_context"])
        );
    }
    ...
    fn single_request_round_trip_decrypts_native_like_report_and_response() {
        ...
        let server = thread::spawn(move || {
            ...
            assert!(headers.contains("x-native-generation: 2026071502"));
