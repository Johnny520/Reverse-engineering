...
public final class hf1 {
    ...
    /* JADX DEBUG: Don't trust debug lines info. Lines numbers was adjusted: min line is 1 */
    public static final void c(float[] fArr, qk1 qk1Var) {
        if (fArr.length < 16) {
            return;
        }
        float f = fArr[0];
        ...
        qk1Var.a = Math.min(f19, Math.min(f26, Math.min(f32, f37)));
        qk1Var.b = Math.min(f22, Math.min(f28, Math.min(f34, f38)));
        qk1Var.c = Math.max(f19, Math.max(f26, Math.max(f32, f37)));
        qk1Var.d = Math.max(f22, Math.max(f28, Math.max(f34, f38)));
    }
