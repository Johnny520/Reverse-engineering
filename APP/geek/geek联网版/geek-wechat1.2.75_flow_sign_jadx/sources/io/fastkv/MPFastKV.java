...
public final class MPFastKV extends AbsFastKV {
    ...
    public MPFastKV(String str, String str2, FastEncoder[] fastEncoderArr, FastCipher fastCipher, boolean z) {
        ...
        this.aFile = new File(str, g40.m1146i(str2, ".kva"));
        this.bFile = new File(str, g40.m1146i(str2, ".kvb"));
        this.needWatchFileChange = z;
        synchronized (this.data) {
            FastKVConfig.getExecutor().execute(new RunnableC0496ms(this, 0));
    ...
    private void fullWriteBufferToA() {
        try {
            if (alignAToBuffer()) {
                ...
                this.aBuffer.put(this.fastBuffer.f2561hb, 0, this.dataEnd);
            }
        } catch (Exception e) {
            error(e);
        }
    ...
    */
    ...
    private void reloadData() {
        ...
        } else if (fastBuffer.f2561hb.length != iCapacity) {
            fastBuffer.f2561hb = new byte[iCapacity];
        }
        this.aBuffer.rewind();
        ...
        try {
            try {
                ...
                if (!this.deletedFiles.isEmpty()) {
                    ...
                    while (it.hasNext()) {
                        FastKVConfig.getExecutor().execute(new RunnableC0732t5(this, MSG_CLEAR, it.next()));
                    }
                }
                if (this.fastBuffer.f2561hb.length - this.dataEnd > AbsFastKV.TRUNCATE_THRESHOLD) {
                    truncate();
                }
                this.updateCount = 0;
                if (!this.deletedFiles.isEmpty()) {
    ...
    public void apply() {
        this.applyExecutor.execute(new RunnableC0496ms(this, 1));
    }
