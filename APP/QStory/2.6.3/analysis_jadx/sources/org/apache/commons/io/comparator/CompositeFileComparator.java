...
public class CompositeFileComparator extends AbstractC5628 implements Serializable {
    ...
    public CompositeFileComparator(Iterable<Comparator<File>> iterable) {
        this.delegates = iterable == null ? emptyArray() : (Comparator[]) StreamSupport.stream(iterable.spliterator(), false).toArray(new C2602(3));
    }
    ...
    public int compare(File file, File file2) {
        return ((Integer) Stream.of((Object[]) this.delegates).map(new C5627(file, file2, 0)).filter(new C2603(2)).findFirst().orElse(0)).intValue();
    }
