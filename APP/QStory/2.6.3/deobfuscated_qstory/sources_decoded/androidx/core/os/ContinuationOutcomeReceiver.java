...
final class ContinuationOutcomeReceiver<R, E extends Throwable> extends AtomicBoolean implements OutcomeReceiver {
    private final InterfaceC4357<R> continuation;

    /* JADX WARN: Multi-variable type inference failed */
    public ContinuationOutcomeReceiver(InterfaceC4357<? super R> interfaceC4357) {
        super(false);
    ...
    public void onError(E error) {
        if (compareAndSet(false, true)) {
            this.continuation.resumeWith(Result.m8745constructorimpl(AbstractC5185.m10211(error)));
        }
    }

    public void onResult(R result) {
        if (compareAndSet(false, true)) {
            this.continuation.resumeWith(Result.m8745constructorimpl(result));
        }
    }
