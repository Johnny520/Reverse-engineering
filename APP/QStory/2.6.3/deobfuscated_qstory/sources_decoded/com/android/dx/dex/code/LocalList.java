...
public final class LocalList extends FixedSizeList {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (entry.isStart()) {
                ...
                if (entryArr[register] == null) {
                    C2942.m6402(Integer.toHexString(entry.getAddress()), "redundant end at ");
                    return;
                }
                ...
                for (int i2 = i + 1; i2 < size; i2++) {
                    ...
                    if (entry3.getRegisterSpec().getReg() == register) {
                        if (!entry3.isStart()) {
                            C2942.m6402(Integer.toHexString(address), "redundant end at ");
                            return;
                        } else {
                            if (entry.getDisposition() != Disposition.END_REPLACED) {
                                C2942.m6402(Integer.toHexString(address), "improperly marked end at ");
                                return;
                            }
                            z = true;
                        }
                ...
                if (!z && entry.getDisposition() == Disposition.END_REPLACED) {
                    C2942.m6402(Integer.toHexString(address), "improper end replacement claim at ");
                    return;
                }
    ...
    public static class Entry implements Comparable<Entry> {
        ...
        public Entry(int i, Disposition disposition, RegisterSpec registerSpec) {
            if (i < 0) {
                C5925.m11310("address < 0");
                throw null;
            }
            if (disposition == null) {
                C2633.m5343("disposition == null");
                throw null;
            }
            try {
