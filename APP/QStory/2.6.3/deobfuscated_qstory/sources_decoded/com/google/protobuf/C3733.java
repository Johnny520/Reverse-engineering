package com.google.protobuf;

import sun.misc.Unsafe;

/* JADX INFO: renamed from: com.google.protobuf.飘花落叶言苏楪子兰世哲, reason: contains not printable characters */
/* JADX INFO: compiled from: r8-map-id-447c03deab370cabd87f71de7ff996ccc1a6dc9764ce389c731d875d052048e4 */
/* JADX INFO: loaded from: classes.dex */
public final class C3733 extends AbstractC3727 {

    /* JADX INFO: renamed from: 飘花落叶言子楪世苏兰哲, reason: contains not printable characters */
    public final /* synthetic */ int f11573;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public /* synthetic */ C3733(Unsafe unsafe, int i) {
        super(unsafe);
        this.f11573 = i;
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪世兰哲苏 */
    public final double mo8001(Object obj, long j) {
        switch (this.f11573) {
        }
        return Double.longBitsToDouble(m8021(obj, j));
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪世兰苏哲 */
    public final byte mo8002(Object obj, long j) {
        switch (this.f11573) {
            case 0:
                if (!AbstractC3726.f11551) {
                }
                break;
            default:
                if (!AbstractC3726.f11551) {
                }
                break;
        }
        return AbstractC3726.m7999(obj, j);
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪世哲兰苏 */
    public final byte mo8003(long j) {
        switch (this.f11573) {
            case 0:
                throw new UnsupportedOperationException();
            default:
                throw new UnsupportedOperationException();
        }
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪世哲苏兰 */
    public final boolean mo8004(Object obj, long j) {
        switch (this.f11573) {
            case 0:
                if (AbstractC3726.f11551) {
                    if (AbstractC3726.m7995(obj, j) == 0) {
                    }
                } else if (AbstractC3726.m7999(obj, j) == 0) {
                }
                break;
            default:
                if (AbstractC3726.f11551) {
                    if (AbstractC3726.m7995(obj, j) == 0) {
                    }
                } else if (AbstractC3726.m7999(obj, j) == 0) {
                }
                break;
        }
        return false;
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪兰苏世哲 */
    public final boolean mo8009() {
        switch (this.f11573) {
        }
        return false;
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪哲世兰苏 */
    public final void mo8010(Object obj, long j, byte b) {
        switch (this.f11573) {
            case 0:
                if (!AbstractC3726.f11551) {
                    AbstractC3726.m7992(obj, j, b);
                } else {
                    AbstractC3726.m7998(obj, j, b);
                }
                break;
            default:
                if (!AbstractC3726.f11551) {
                    AbstractC3726.m7992(obj, j, b);
                } else {
                    AbstractC3726.m7998(obj, j, b);
                }
                break;
        }
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪哲世苏兰 */
    public final void mo8011(Object obj, long j, boolean z) {
        switch (this.f11573) {
            case 0:
                if (!AbstractC3726.f11551) {
                    AbstractC3726.m7992(obj, j, z ? (byte) 1 : (byte) 0);
                } else {
                    AbstractC3726.m7998(obj, j, z ? (byte) 1 : (byte) 0);
                }
                break;
            default:
                if (!AbstractC3726.f11551) {
                    AbstractC3726.m7992(obj, j, z ? (byte) 1 : (byte) 0);
                } else {
                    AbstractC3726.m7998(obj, j, z ? (byte) 1 : (byte) 0);
                }
                break;
        }
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪哲苏世兰 */
    public final void mo8014(Object obj, long j, double d) {
        switch (this.f11573) {
            case 0:
                m8013(obj, j, Double.doubleToLongBits(d));
                break;
            default:
                m8013(obj, j, Double.doubleToLongBits(d));
                break;
        }
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪哲苏兰世 */
    public final void mo8015(Object obj, long j, float f) {
        switch (this.f11573) {
            case 0:
                m8012(obj, j, Float.floatToIntBits(f));
                break;
            default:
                m8012(obj, j, Float.floatToIntBits(f));
                break;
        }
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪苏世哲兰 */
    public final float mo8017(Object obj, long j) {
        switch (this.f11573) {
        }
        return Float.intBitsToFloat(m8016(obj, j));
    }

    @Override // com.google.protobuf.AbstractC3727
    /* JADX INFO: renamed from: 飘花落叶言子楪苏哲世兰 */
    public final long mo8020(long j) {
        switch (this.f11573) {
            case 0:
                throw new UnsupportedOperationException();
            default:
                throw new UnsupportedOperationException();
        }
    }
}
