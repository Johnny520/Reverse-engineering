...
public class CachedReturnPlugin extends Plugin.ForElementMatcher implements Plugin.Factory {
    private static final MethodDescription.InDefinedShape ENHANCE_VALUE = (MethodDescription.InDefinedShape) TypeDescription.ForLoadedType.m289of(Enhance.class).getDeclaredMethods().filter(ElementMatchers.named("value")).getOnly();
    ...
    public enum AdviceResolver {
        ...
        AdviceResolver(Class cls, int i, int i2, int i3, int i4) {
            ...
            this.dynamicType = builderName.defineMethod("enter", cls, ownership).withParameter(cls).annotateParameter(AnnotationDescription.Builder.ofType((Class<? extends Annotation>) CacheField.class).build()).intercept(new Implementation.Simple(MethodVariableAccess.m449of(TypeDescription.ForLoadedType.m289of(cls)).loadFrom(0), MethodReturn.m448of(TypeDescription.ForLoadedType.m289of(cls)))).annotateMethod(AnnotationDescription.Builder.ofType((Class<? extends Annotation>) Advice.OnMethodEnter.class).define("skipOn", Advice.OnNonDefaultValue.class).build()).defineMethod("exit", Void.TYPE, ownership).withParameter(cls).annotateParameter(AnnotationDescription.Builder.ofType((Class<? extends Annotation>) Advice.Return.class).define("readOnly", false).define("typing", Assigner.Typing.DYNAMIC).build()).withParameter(cls).annotateParameter(AnnotationDescription.Builder.ofType((Class<? extends Annotation>) CacheField.class).build()).intercept(new Implementation.Simple(new ExitAdviceByteCodeAppender(i, i2, i3, i4, StackSize.m436of((Class<?>) cls).getSize()))).annotateMethod(AnnotationDescription.Builder.ofType((Class<? extends Annotation>) Advice.OnMethodExit.class).build()).make();
        }

        /* JADX INFO: renamed from: of */
        public static AdviceResolver m241of(TypeDefinition typeDefinition) {
            ...
            C5043.m9163(typeDefinition, "Unexpected advice type: ");
            return null;
        }

        public Advice toAdvice(String str) {
            return Advice.withCustomMapping().bind(CacheField.class, (Advice.OffsetMapping) new CacheFieldOffsetMapping(str)).m219to(this.dynamicType.getTypeDescription(), this.dynamicType);
        }
    }
    ...
    public DynamicType.Builder<?> apply(DynamicType.Builder<?> builder, TypeDescription typeDescription, ClassFileLocator classFileLocator) {
        for (MethodDescription.InDefinedShape inDefinedShape : typeDescription.getDeclaredMethods().filter(ElementMatchers.not(ElementMatchers.isBridge()).and(ElementMatchers.isAnnotatedWith((Class<? extends Annotation>) Enhance.class)))) {
            if (inDefinedShape.isAbstract()) {
                C8376.m13333(inDefinedShape, "Cannot cache the value of an abstract method: ");
                return null;
            }
            if (!inDefinedShape.getParameters().isEmpty()) {
                C8376.m13333(inDefinedShape, "Cannot cache the value of a method with parameters: ");
                return null;
            }
            if (inDefinedShape.getReturnType().represents(Void.TYPE)) {
                C8376.m13333(inDefinedShape, "Cannot cache void result for ");
                return null;
            }
