package top.suzhelan.qstory.p015ui.activity;

import kotlin.C6008;
import kotlin.Metadata;
import kotlin.collections.EmptyList;
import kotlin.jvm.internal.FunctionReferenceImpl;
import kotlinx.coroutines.flow.C6099;
import p025.C6999;
import p068.InterfaceC7372;
import p366.C9682;
import p366.C9688;
import p407.C9924;

/* JADX INFO: compiled from: r8-map-id-447c03deab370cabd87f71de7ff996ccc1a6dc9764ce389c731d875d052048e4 */
/* JADX INFO: loaded from: classes2.dex */
@Metadata(m152k = 3, m153mv = {2, 3, 0}, m155xi = 50)
final /* synthetic */ class SettingActivityKt$SettingRoute$1$2$6$1 extends FunctionReferenceImpl implements InterfaceC7372 {
    public SettingActivityKt$SettingRoute$1$2$6$1(Object obj) {
        super(0, obj, C9688.class, "onSearchHistoryCleared", "onSearchHistoryCleared()V", 0);
    }

    /* JADX INFO: renamed from: invoke, reason: collision with other method in class */
    public final void m11895invoke() {
        Object value;
        C9688 c9688 = (C9688) this.receiver;
        c9688.getClass();
        C6999.f17312.getClass();
        C9924 c9924 = C6999.f17311;
        String strM14532 = "history";
        c9924.getClass();
        c9924.f25834.remove(strM14532);
        C6099 c6099 = c9688.f25324;
        do {
            value = c6099.getValue();
        } while (!c6099.m10871(value, C9682.m15079((C9682) value, null, 0, 0, false, null, null, EmptyList.INSTANCE, null, 383)));
    }

    @Override // p068.InterfaceC7372
    public /* bridge */ /* synthetic */ Object invoke() {
        m11895invoke();
        return C6008.f15084;
    }
}
