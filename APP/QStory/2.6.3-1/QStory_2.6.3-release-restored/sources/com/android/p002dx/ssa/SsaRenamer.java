...
public class SsaRenamer implements Runnable {
    ...
    public class BlockRenamer implements SsaInsn.Visitor {
        ...
        public class RenamingMapper extends RegisterMapper {
            ...
            @Override // com.android.p002dx.ssa.RegisterMapper
        ...
        public void process() {
            ...
            for (SsaBasicBlock ssaBasicBlock : this.block.getDomChildren()) {
                if (ssaBasicBlock != this.block) {
                    RegisterSpec[] registerSpecArrDupArray = this.currentMapping;
                    if (!z) {
                        registerSpecArrDupArray = SsaRenamer.dupArray(registerSpecArrDupArray);
                    }
                    SsaRenamer.this.startsForBlocks[ssaBasicBlock.getIndex()] = registerSpecArrDupArray;
                    z = false;
                }
            }
        }
    ...
    public void setNameForSsaReg(RegisterSpec registerSpec) {
        ...
        while (true) {
            int size = this.ssaRegToLocalItems.size();
            ArrayList<LocalItem> arrayList = this.ssaRegToLocalItems;
            if (size > reg) {
                arrayList.set(reg, localItem);
                return;
            }
            arrayList.add(null);
        }
