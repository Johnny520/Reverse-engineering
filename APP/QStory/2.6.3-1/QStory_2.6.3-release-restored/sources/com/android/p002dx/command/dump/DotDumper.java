...
public class DotDumper implements ParseObserver {
    ...
    @Override // com.android.p002dx.p003cf.iface.ParseObserver
    public void endParsingMember(ByteArray byteArray, int i, String str, String str2, Member member) {
        if ((member instanceof Method) && shouldDumpMethod(str)) {
            ...
            System.out.println("\tfirst -> n" + Hex.m28u2(ropMethodConvert.getFirstLabel()) + ";");
            ...
            for (int i2 = 0; i2 < size; i2++) {
                ...
                if (successors.size() == 0) {
                    System.out.println("\tn" + Hex.m28u2(label) + " -> returns;");
                } else if (successors.size() == 1) {
                    System.out.println("\tn" + Hex.m28u2(label) + " -> n" + Hex.m28u2(successors.get(0)) + ";");
                } else {
                    System.out.print("\tn" + Hex.m28u2(label) + " -> {");
                    for (int i3 = 0; i3 < successors.size(); i3++) {
                        int i4 = successors.get(i3);
                        if (i4 != basicBlock.getPrimarySuccessor()) {
                            System.out.print(" n" + Hex.m28u2(i4) + " ");
                        }
                    }
                    System.out.println("};");
                    System.out.println("\tn" + Hex.m28u2(label) + " -> n" + Hex.m28u2(basicBlock.getPrimarySuccessor()) + " [label=\"primary\"];");
                }
            }
            System.out.println("}");
        }
