        ...
        for (int i = 0; i < size; i++) {
            ...
            if (entry.isStart()) {
                ...
                if (entryArr[register] == null) {
                    C3775.m6962(Integer.toHexString(entry.getAddress()), "redundant end at ");
                    return;
                }
                ...
                for (int i2 = i + 1; i2 < size; i2++) {
                    ...
                    if (entry3.getRegisterSpec().getReg() == register) {
                        if (!entry3.isStart()) {
                            C3775.m6962(Integer.toHexString(address), "redundant end at ");
                            return;
                        } else {
                            if (entry.getDisposition() != Disposition.END_REPLACED) {
                                C3775.m6962(Integer.toHexString(address), "improperly marked end at ");
                                return;
                            }
                            z = true;
                        }
                ...
                if (!z && entry.getDisposition() == Disposition.END_REPLACED) {
                    C3775.m6962(Integer.toHexString(address), "improper end replacement claim at ");
                    return;
                }
