...
public class RebaseDynamicTypeBuilder<T> extends AbstractInliningDynamicTypeBuilder<T> {
    ...
    public TypeWriter<T> toTypeWriter(TypePool typePool) {
        MethodRegistry.Prepared preparedPrepare = this.methodRegistry.prepare(this.instrumentedType, this.methodGraphCompiler, this.typeValidation, this.visibilityBridgeStrategy, InliningImplementationMatcher.m353of(this.ignoredMethods, this.originalType));
        HashSet hashSet = new HashSet(this.originalType.getDeclaredMethods().asSignatureTokenList(ElementMatchers.m452is(this.originalType), this.instrumentedType));
