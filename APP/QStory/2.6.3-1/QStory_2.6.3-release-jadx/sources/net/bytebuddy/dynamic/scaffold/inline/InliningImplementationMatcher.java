...
public class InliningImplementationMatcher implements LatentMatcher<MethodDescription> {
    ...
    public static LatentMatcher<MethodDescription> m353of(LatentMatcher<? super MethodDescription> latentMatcher, TypeDescription typeDescription) {
        ...
        for (MethodDescription.InDefinedShape inDefinedShape : typeDescription.getDeclaredMethods()) {
            junctionNone = junctionNone.mo451or((inDefinedShape.isConstructor() ? ElementMatchers.isConstructor() : ElementMatchers.named(inDefinedShape.getName())).and(ElementMatchers.returns(inDefinedShape.getReturnType().asErasure())).and(ElementMatchers.takesArguments(inDefinedShape.getParameters().asTypeList().asErasures())));
        }
