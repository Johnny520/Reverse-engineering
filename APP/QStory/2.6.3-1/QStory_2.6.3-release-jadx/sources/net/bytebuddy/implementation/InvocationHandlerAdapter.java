...
public abstract class InvocationHandlerAdapter implements Implementation.Composable {
    ...
    private static final TypeDescription.Generic INVOCATION_HANDLER_TYPE = TypeDescription.Generic.OfNonGenericType.ForLoadedType.m296of(InvocationHandler.class);
    ...
    private List<StackManipulation> argumentValuesOf(MethodDescription methodDescription) {
        ...
        for (TypeDescription.Generic generic : genericAsTypeList) {
            arrayList.add(new StackManipulation.Compound(MethodVariableAccess.m449of(generic).loadFrom(size), this.assigner.assign(generic, TypeDescription.Generic.OfNonGenericType.ForLoadedType.m296of(Object.class), Assigner.Typing.STATIC)));
    ...
    public static InvocationHandlerAdapter m374of(InvocationHandler invocationHandler) {
        return m375of(invocationHandler, "invocationHandler$" + RandomString.hashOf(invocationHandler));
    }
    ...
    public ByteCodeAppender.Size apply(MethodVisitor methodVisitor, Implementation.Context context, MethodDescription methodDescription, StackManipulation stackManipulation, FieldDescription fieldDescription) {
        if (methodDescription.isStatic() || methodDescription.isConstructor()) {
            C6755.m11870(AbstractC3275.m5133("It is not possible to apply an invocation handler onto the static method or constructor ", methodDescription));
            return null;
        }
        MethodConstant.CanCache canCacheOfPrivileged = this.privileged ? MethodConstant.ofPrivileged(methodDescription.asDefined()) : MethodConstant.m444of(methodDescription.asDefined());
        ...
        return new ByteCodeAppender.Size(new StackManipulation.Compound(stackManipulation, stackManipulation2, stackManipulationLoadThis, canCacheCached, methodDescription.getParameters().isEmpty() ? NullConstant.INSTANCE : ArrayFactory.forType(TypeDescription.Generic.OfNonGenericType.ForLoadedType.m296of(Object.class)).withValues(argumentValuesOf(methodDescription)), MethodInvocation.invoke((MethodDescription) INVOCATION_HANDLER_TYPE.getDeclaredMethods().filter(ElementMatchers.isAbstract()).getOnly()), this.returning ? new StackManipulation.Compound(this.assigner.assign(TypeDescription.Generic.OfNonGenericType.ForLoadedType.m296of(Object.class), methodDescription.getReturnType(), Assigner.Typing.DYNAMIC), MethodReturn.m448of(methodDescription.getReturnType())) : Removal.SINGLE).apply(methodVisitor, context).getMaximalSize(), methodDescription.getStackSize());
    }
    ...
    public int hashCode() {
        return this.assigner.hashCode() + ((((((AbstractC7012.m12131(getClass().hashCode() * 31, 31, this.fieldName) + (this.cached ? 1 : 0)) * 31) + (this.privileged ? 1 : 0)) * 31) + (this.returning ? 1 : 0)) * 31);
    }
    ...
    /* JADX INFO: renamed from: of */
    public static InvocationHandlerAdapter m375of(InvocationHandler invocationHandler, String str) {
    ...
    public static class ForField extends InvocationHandlerAdapter implements WithoutPrivilegeConfiguration {
        ...
        public ByteCodeAppender appender(Implementation.Target target) {
            ...
            if (!resolutionLocate.isResolved()) {
                C1123.m1407("Could not find a field named '", this.fieldName, "' for ", target.getInstrumentedType());
                return null;
            }
            ...
            C3141.m4914(resolutionLocate.getField(), "Field ", " does not declare a type that is assignable to invocation handler");
            return null;
        }
    ...
    public static class ForInstance extends InvocationHandlerAdapter implements WithoutPrivilegeConfiguration {
        ...
        public class Appender implements ByteCodeAppender {
            ...
            public int hashCode() {
                return ForInstance.this.hashCode() + AbstractC3275.m5144(this.instrumentedType, getClass().hashCode() * 31, 31);
            }
        }
        ...
        @Override // net.bytebuddy.dynamic.scaffold.InstrumentedType.Prepareable
        public InstrumentedType prepare(InstrumentedType instrumentedType) {
            boolean zIsEmpty = instrumentedType.getDeclaredFields().filter(ElementMatchers.named(this.fieldName).and(ElementMatchers.fieldType(InvocationHandlerAdapter.INVOCATION_HANDLER_TYPE.asErasure()))).isEmpty();
            String str = this.fieldName;
            if (zIsEmpty) {
                return instrumentedType.withField(new FieldDescription.Token(str, 4169, InvocationHandlerAdapter.INVOCATION_HANDLER_TYPE)).withInitializer(new LoadedTypeInitializer.ForStaticField(this.fieldName, this.invocationHandler));
            }
            C3141.m4912(AbstractC7012.m12144("Field with name ", str, " and type "), InvocationHandlerAdapter.INVOCATION_HANDLER_TYPE.asErasure(), " already declared by ", instrumentedType);
            return null;
        }
