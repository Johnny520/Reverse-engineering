        ...
        public MethodDelegationBinder.ParameterBinding<?> bind(AnnotationDescription.Loadable<AllArguments> loadable, MethodDescription methodDescription, ParameterDescription parameterDescription, Implementation.Target target, Assigner assigner, Assigner.Typing typing) {
            ...
            if (parameterDescription.getType().represents(Object.class)) {
                ...
                if (!parameterDescription.getType().isArray()) {
                    C6755.m11870(AbstractC3275.m5133("Expected an array type for all argument annotation on ", methodDescription));
                    return null;
                }
            ...
            for (TypeDescription.Generic generic : i != 0 ? CompoundList.m475of(target.getInstrumentedType().asGenericType(), methodDescription.getParameters().asTypeList()) : methodDescription.getParameters().asTypeList()) {
                StackManipulation.Compound compound = new StackManipulation.Compound(MethodVariableAccess.m449of(generic).loadFrom(size), assigner.assign(generic, componentType, typing));
                if (compound.isValid()) {
