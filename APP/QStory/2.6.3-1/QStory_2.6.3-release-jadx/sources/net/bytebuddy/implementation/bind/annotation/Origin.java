...
public @interface Origin {
    ...
    public enum Binder implements TargetMethodAnnotationDrivenBinder.ParameterBinder<Origin> {
        ...
        static {
            MethodList<MethodDescription.InDefinedShape> declaredMethods = TypeDescription.ForLoadedType.m289of(Origin.class).getDeclaredMethods();
        ...
        private static StackManipulation methodConstant(AnnotationDescription.Loadable<Origin> loadable, MethodDescription.InDefinedShape inDefinedShape) {
            MethodConstant.CanCache canCacheOfPrivileged = ((Boolean) loadable.getValue(PRIVILEGED).resolve(Boolean.class)).booleanValue() ? MethodConstant.ofPrivileged(inDefinedShape) : MethodConstant.m444of(inDefinedShape);
        ...
        public MethodDelegationBinder.ParameterBinding<?> bind(AnnotationDescription.Loadable<Origin> loadable, MethodDescription methodDescription, ParameterDescription parameterDescription, Implementation.Target target, Assigner assigner, Assigner.Typing typing) {
            ...
            if (typeDescriptionAsErasure.represents(Class.class)) {
                return new MethodDelegationBinder.ParameterBinding.Anonymous(ClassConstant.m442of(target.getOriginType().asErasure()));
            }
            ...
            if (typeDescriptionAsErasure.equals(JavaType.METHOD_HANDLE.getTypeStub())) {
                return new MethodDelegationBinder.ParameterBinding.Anonymous(JavaConstant.MethodHandle.m481of(methodDescription.asDefined()).toStackManipulation());
            }
            if (typeDescriptionAsErasure.equals(JavaType.METHOD_TYPE.getTypeStub())) {
                return new MethodDelegationBinder.ParameterBinding.Anonymous(JavaConstant.MethodType.m487of(methodDescription.asDefined()).toStackManipulation());
            }
            if (typeDescriptionAsErasure.equals(JavaType.METHOD_HANDLES_LOOKUP.getTypeStub())) {
            ...
            C6385.m11436(sb, " method's ", parameterDescription.getIndex(), " parameter is annotated with a Origin annotation with an argument not representing a Class, Method, Constructor, String, int, MethodType or MethodHandle type");
            return null;
        }
