package androidx.collection;

import kotlin.AbstractC6017;
import kotlin.C6008;
import kotlin.Metadata;
import kotlin.coroutines.InterfaceC5189;
import kotlin.coroutines.intrinsics.CoroutineSingletons;
import kotlin.coroutines.jvm.internal.RestrictedSuspendLambda;
import kotlin.sequences.AbstractC5957;
import p068.InterfaceC7383;
import p079.InterfaceC7691;
import top.suzhelan.qstory.hook.item.C6755;

/* JADX INFO: compiled from: r8-map-id-447c03deab370cabd87f71de7ff996ccc1a6dc9764ce389c731d875d052048e4 */
/* JADX INFO: loaded from: classes.dex */
@InterfaceC7691(m555c = "androidx.collection.Entries$iterator$1", m556f = "ScatterMap.kt", m557l = {1414}, m558m = "invokeSuspend")
@Metadata(m150d1 = {"\u0000\u0014\n\u0002\b\u0002\n\u0002\u0018\u0002\n\u0002\u0010&\n\u0002\u0018\u0002\n\u0002\b\u0002\u0010\u0005\u001a\u00020\u0004\"\u0004\b\u0000\u0010\u0000\"\u0004\b\u0001\u0010\u0001*\u0014\u0012\u0010\u0012\u000e\u0012\u0004\u0012\u00028\u0000\u0012\u0004\u0012\u00028\u00010\u00030\u0002H\u008a@¢\u0006\u0004\b\u0005\u0010\u0006"}, m151d2 = {"K", "V", "Lkotlin/sequences/飘花落叶言子楪苏兰世哲;", "", "Lkotlin/飘花落叶言子楪兰苏哲世;", "<anonymous>", "(Lkotlin/sequences/飘花落叶言子楪苏兰世哲;)V"}, m152k = 3, m153mv = {1, 9, 0})
final class Entries$iterator$1 extends RestrictedSuspendLambda implements InterfaceC7383 {
    int I$0;
    int I$1;
    int I$2;
    int I$3;
    long J$0;
    private /* synthetic */ Object L$0;
    Object L$1;
    Object L$2;
    int label;
    final /* synthetic */ C1126 this$0;

    /* JADX WARN: 'super' call moved to the top of the method (can break code semantics) */
    public Entries$iterator$1(C1126 c1126, InterfaceC5189<? super Entries$iterator$1> interfaceC5189) {
        super(2, interfaceC5189);
        this.this$0 = c1126;
    }

    @Override // kotlin.coroutines.jvm.internal.BaseContinuationImpl
    public final InterfaceC5189<C6008> create(Object obj, InterfaceC5189<?> interfaceC5189) {
        Entries$iterator$1 entries$iterator$1 = new Entries$iterator$1(this.this$0, interfaceC5189);
        entries$iterator$1.L$0 = obj;
        return entries$iterator$1;
    }

    @Override // p068.InterfaceC7383
    public final Object invoke(AbstractC5957 abstractC5957, InterfaceC5189<? super C6008> interfaceC5189) {
        return ((Entries$iterator$1) create(abstractC5957, interfaceC5189)).invokeSuspend(C6008.f15084);
    }

    /* JADX WARN: Removed duplicated region for block: B:13:0x0053  */
    /* JADX WARN: Removed duplicated region for block: B:15:0x0067  */
    /* JADX WARN: Removed duplicated region for block: B:22:0x00a8  */
    /* JADX WARN: Removed duplicated region for block: B:25:0x00b4  */
    /* JADX WARN: Removed duplicated region for block: B:27:0x00bb  */
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:13:0x0053 -> B:14:0x0065). Please report as a decompilation issue!!! */
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:16:0x006e -> B:20:0x009e). Please report as a decompilation issue!!! */
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:18:0x009b -> B:21:0x00a1). Please report as a decompilation issue!!! */
    /* JADX WARN: Unsupported multi-entry loop pattern (BACK_EDGE: B:25:0x00b4 -> B:26:0x00b9). Please report as a decompilation issue!!! */
    @Override // kotlin.coroutines.jvm.internal.BaseContinuationImpl
    /*
        Code decompiled incorrectly, please refer to instructions dump.
    */
    public final Object invokeSuspend(Object obj) {
        AbstractC5957 abstractC5957;
        C1126 c1126;
        long[] jArr;
        int length;
        int i;
        long j;
        CoroutineSingletons coroutineSingletons = CoroutineSingletons.COROUTINE_SUSPENDED;
        int i2 = this.label;
        int i3 = 8;
        int i4 = 1;
        if (i2 == 0) {
            AbstractC6017.m10769(obj);
            abstractC5957 = (AbstractC5957) this.L$0;
            c1126 = this.this$0;
            jArr = c1126.f1332.f1226;
            length = jArr.length - 2;
            if (length >= 0) {
                i = 0;
                j = jArr[i];
                if ((((~j) << 7) & j & (-9187201950435737472L)) == -9187201950435737472L) {
                }
            }
            return C6008.f15084;
        }
        if (i2 != 1) {
            C6755.m11870("call to 'resume' before 'invoke' with coroutine");
            return null;
        }
        int i5 = this.I$3;
        int i6 = this.I$2;
        long j2 = this.J$0;
        i = this.I$1;
        int i7 = this.I$0;
        long[] jArr2 = (long[]) this.L$2;
        C1126 c11262 = (C1126) this.L$1;
        AbstractC5957 abstractC59572 = (AbstractC5957) this.L$0;
        AbstractC6017.m10769(obj);
        int i8 = i3;
        int i9 = i4;
        j2 >>= i8;
        i5 += i9;
        i4 = i9;
        i3 = i8;
        if (i5 < i6) {
            int i10 = i4;
            int i11 = i3;
            int i12 = i10;
            if (i6 == i11) {
                length = i7;
                jArr = jArr2;
                c1126 = c11262;
                abstractC5957 = abstractC59572;
                if (i != length) {
                    i++;
                    int i13 = i11;
                    i4 = i12;
                    i3 = i13;
                    j = jArr[i];
                    if ((((~j) << 7) & j & (-9187201950435737472L)) == -9187201950435737472L) {
                        c11262 = c1126;
                        i6 = 8 - ((~(i - length)) >>> 31);
                        abstractC59572 = abstractC5957;
                        i5 = 0;
                        jArr2 = jArr;
                        i7 = length;
                        j2 = j;
                        if (i5 < i6) {
                            if ((255 & j2) < 128) {
                                int i14 = (i << 3) + i5;
                                i8 = i3;
                                C1083 c1083 = c11262.f1332;
                                C1112 c1112 = new C1112(c1083.f1225[i14], 0, c1083.f1224[i14]);
                                this.L$0 = abstractC59572;
                                this.L$1 = c11262;
                                this.L$2 = jArr2;
                                this.I$0 = i7;
                                this.I$1 = i;
                                this.J$0 = j2;
                                this.I$2 = i6;
                                this.I$3 = i5;
                                i9 = 1;
                                this.label = 1;
                                if (abstractC59572.mo10680(c1112, this) == coroutineSingletons) {
                                    return coroutineSingletons;
                                }
                            } else {
                                int i82 = i3;
                                int i92 = i4;
                            }
                            j2 >>= i82;
                            i5 += i92;
                            i4 = i92;
                            i3 = i82;
                            if (i5 < i6) {
                            }
                        }
                    } else {
                        int i15 = i4;
                        i11 = i3;
                        i12 = i15;
                        if (i != length) {
                        }
                    }
                }
            }
            return C6008.f15084;
        }
    }
}
