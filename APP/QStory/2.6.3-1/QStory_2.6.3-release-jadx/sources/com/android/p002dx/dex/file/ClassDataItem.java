...
import com.android.p002dx.rop.cst.Zeroes;
...
public final class ClassDataItem extends OffsettedItem {
    ...
    private static void encodeSize(DexFile dexFile, AnnotatedOutput annotatedOutput, String str, int i) {
        if (annotatedOutput.annotates()) {
            annotatedOutput.annotate(String.format("  %-21s %08x", AbstractC0900.m696(str, "_size:"), Integer.valueOf(i)));
        }
    ...
    public void addStaticField(EncodedField encodedField, Constant constant) {
        if (encodedField == null) {
            ...
            C6755.m11867("static fields already sorted");
        } else {
    ...
    public void debugPrint(Writer writer, boolean z) {
        ...
        int size = this.staticFields.size();
        for (int i = 0; i < size; i++) {
            StringBuilder sbM710 = AbstractC0900.m710(i, "  sfields[", "]: ");
            sbM710.append(this.staticFields.get(i));
            printWriterPrintWriterFor.println(sbM710.toString());
        }
        int size2 = this.instanceFields.size();
        for (int i2 = 0; i2 < size2; i2++) {
            StringBuilder sbM7102 = AbstractC0900.m710(i2, "  ifields[", "]: ");
            sbM7102.append(this.instanceFields.get(i2));
            printWriterPrintWriterFor.println(sbM7102.toString());
        }
