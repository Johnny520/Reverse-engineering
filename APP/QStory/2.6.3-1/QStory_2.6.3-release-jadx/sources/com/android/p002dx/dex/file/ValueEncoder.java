...
import com.android.p002dx.rop.cst.CstKnownNull;
...
public final class ValueEncoder {
    ...
    public void writeAnnotation(Annotation annotation, boolean z) {
        ...
        if (z2) {
            this.out.annotate("  type_idx: " + Hex.m30u4(iIndexOf) + " // " + type.toHuman());
        }
        ...
        if (z2) {
            this.out.annotate("  size: " + Hex.m30u4(size));
        }
        ...
        for (NameValuePair nameValuePair : nameValuePairs) {
            ...
            if (z2) {
                ...
                i++;
                this.out.annotate("    name_idx: " + Hex.m30u4(iIndexOf2) + " // " + name.toHuman());
            }
    ...
    public void writeConstant(Constant constant) {
        ...
        if (iConstantToValueType != 0 && iConstantToValueType != 6 && iConstantToValueType != 2) {
            if (iConstantToValueType == 3) {
                AbstractC3888.m7283(this.out, iConstantToValueType, ((CstLiteralBits) constant).getLongBits());
                return;
            }
            if (iConstantToValueType != 4) {
                if (iConstantToValueType == 16) {
                    AbstractC3888.m7288(this.out, iConstantToValueType, ((CstFloat) constant).getLongBits() << 32);
                    return;
                }
                if (iConstantToValueType == 17) {
                    AbstractC3888.m7288(this.out, iConstantToValueType, ((CstDouble) constant).getLongBits());
                    return;
                }
                switch (iConstantToValueType) {
                    case 21:
                        AbstractC3888.m7283(this.out, iConstantToValueType, this.file.getProtoIds().indexOf(((CstProtoRef) constant).getPrototype()));
                        break;
                    case 22:
                        AbstractC3888.m7283(this.out, iConstantToValueType, this.file.getMethodHandles().indexOf((CstMethodHandle) constant));
                        break;
                    case 23:
                        AbstractC3888.m7283(this.out, iConstantToValueType, this.file.getStringIds().indexOf((CstString) constant));
                        break;
                    case 24:
                        AbstractC3888.m7283(this.out, iConstantToValueType, this.file.getTypeIds().indexOf((CstType) constant));
                        break;
                    case 25:
                        AbstractC3888.m7283(this.out, iConstantToValueType, this.file.getFieldIds().indexOf((CstFieldRef) constant));
                        break;
                    case 26:
                        AbstractC3888.m7283(this.out, iConstantToValueType, this.file.getMethodIds().indexOf((CstMethodRef) constant));
                        break;
                    case 27:
                        AbstractC3888.m7283(this.out, iConstantToValueType, this.file.getFieldIds().indexOf(((CstEnumRef) constant).getFieldRef()));
                        break;
                    case 28:
        ...
        AbstractC3888.m7289(this.out, iConstantToValueType, ((CstLiteralBits) constant).getLongBits());
    }
