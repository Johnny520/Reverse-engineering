...
public class SsaToRop {
    ...
    private BasicBlock convertBasicBlock(SsaBasicBlock ssaBasicBlock) {
        ...
        if (ropLabelSuccessorList.contains(exitBlock == null ? -1 : exitBlock.getRopLabel())) {
            if (ropLabelSuccessorList.size() > 1) {
                C3775.m6962(Hex.m28u2(ssaBasicBlock.getRopLabel()), "Exit predecessor must have no other successors");
                return null;
            }
    ...
    private BasicBlockList convertBasicBlocks() {
        ...
        C3466.m5899("Exit block must have no insns when leaving SSA form");
        return null;
    }
    ...
    private void removeEmptyGotos() {
        ...
        this.ssaMeth.forEachBlockDepthFirst(false, new SsaBasicBlock.Visitor(this) { // from class: com.android.dx.ssa.back.SsaToRop.1
    ...
    private void verifyValidExitPredecessor(SsaBasicBlock ssaBasicBlock) {
        Rop opcode = ((SsaInsn) AbstractC7012.m12145(1, ssaBasicBlock.getInsns())).getOpcode();
        if (opcode.getBranchingness() == 2 || opcode == Rops.THROW) {
            return;
        }
        C3466.m5899("Exit predecessor must end in valid exit statement.");
    }
