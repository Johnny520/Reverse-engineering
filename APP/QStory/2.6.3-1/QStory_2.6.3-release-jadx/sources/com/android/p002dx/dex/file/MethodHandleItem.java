...
public final class MethodHandleItem extends IndexedItem {
    ...
    private int getTargetIndex(DexFile dexFile) {
        ...
        if (!this.methodHandle.isInvocation()) {
            C6755.m11870("Unhandled invocation type");
            return 0;
        }
    ...
    public void writeTo(DexFile dexFile, AnnotatedOutput annotatedOutput) {
        ...
        if (annotatedOutput.annotates()) {
            ...
            annotatedOutput.annotate(2, "type:     " + Hex.m28u2(methodHandleType) + (" // " + CstMethodHandle.getMethodHandleTypeName(methodHandleType)));
            annotatedOutput.annotate(2, "reserved: " + Hex.m28u2(0));
            String str = " // " + this.methodHandle.getRef().toString();
            if (this.methodHandle.isAccessor()) {
                annotatedOutput.annotate(2, "fieldId:  " + Hex.m28u2(targetIndex) + str);
            } else {
                annotatedOutput.annotate(2, "methodId: " + Hex.m28u2(targetIndex) + str);
            }
            annotatedOutput.annotate(2, "reserved: " + Hex.m28u2(0));
        }
