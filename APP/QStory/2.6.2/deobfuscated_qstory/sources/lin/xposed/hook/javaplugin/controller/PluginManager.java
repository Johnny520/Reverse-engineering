...
public class PluginManager {
    ...
    private static boolean checkPluginTime(PluginInfo pluginInfo) {
        ...
        DateTimeFormatterBuilder dateTimeFormatterBuilderAppendPattern = new DateTimeFormatterBuilder().appendPattern("yyyy-");
        ...
        return LocalDate.parse(pluginInfo.getDate(), formatter).isAfter(LocalDate.parse("2025-11-30", formatter));
    }
    ...
    public static synchronized boolean loadPlugin(PluginItemView pluginItemView, PluginInfo pluginInfo) {
        try {
            if (!checkPluginTime(pluginInfo)) {
                ...
                AbstractC6154.m11561("\u811a\u672c:'" + pluginInfo.getPluginName() + "',\u672a\u9002\u914d\u65b0\u7248QStory,\u5728info.prop\u7684date");
                return false;
            }
            ...
            if (map.containsKey(pluginInfo.getPluginID())) {
                ...
                AbstractC6154.m11561("\u52a0\u8f7d\u5931\u8d25 \u5df2\u6709\u76f8\u540c\u811a\u672cID\u5728\u52a0\u8f7d:" + pluginInfo.getPluginID());
                return false;
            }
            try {
    ...
    public static PluginInfo newPluginInfo(File file) {
        ...
        try {
            properties.load(new FileReader(new File(file, "info.prop")));
            pluginInfo.setPluginID(properties.getProperty("id", UUID.randomUUID().toString().replace("-", "")));
            pluginInfo.setPluginName(properties.getProperty("name", null));
            pluginInfo.setPluginAuthor(properties.getProperty("author", null));
            pluginInfo.setPluginVersion(properties.getProperty("version", null));
            pluginInfo.setDate(properties.getProperty("date", null));
            pluginInfo.setTags(properties.getProperty("tags", ""));
            pluginInfo.setPluginLocalPath(file.getAbsolutePath());
            File file2 = new File(file, "desc.txt");
            if (file2.exists()) {
            ...
            if (file3.exists() && file3.isDirectory() && (fileArrListFiles = file3.listFiles()) != null) {
                ...
                while (true) {
                    ...
                    if (file4.getName().startsWith("icon")) {
                ...
                for (File file5 : fileArrListFiles) {
                    if (!file5.getName().startsWith("icon")) {
