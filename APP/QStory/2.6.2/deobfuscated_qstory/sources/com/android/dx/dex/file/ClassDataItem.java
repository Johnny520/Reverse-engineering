...
public final class ClassDataItem extends OffsettedItem {
    ...
    private static void encodeSize(DexFile dexFile, AnnotatedOutput annotatedOutput, String str, int i) {
        if (annotatedOutput.annotates()) {
            annotatedOutput.annotate(String.format("  %-21s %08x", AbstractC0053.m136(str, "_size:"), Integer.valueOf(i)));
        }
    ...
    public void addStaticField(EncodedField encodedField, Constant constant) {
        if (encodedField == null) {
            ...
            C5919.m11247("static fields already sorted");
        } else {
    ...
    public void debugPrint(Writer writer, boolean z) {
        ...
        int size = this.staticFields.size();
        for (int i = 0; i < size; i++) {
            StringBuilder sbM148 = AbstractC0053.m148(i, "  sfields[", "]: ");
            sbM148.append(this.staticFields.get(i));
            printWriterPrintWriterFor.println(sbM148.toString());
        }
        int size2 = this.instanceFields.size();
        for (int i2 = 0; i2 < size2; i2++) {
            StringBuilder sbM1482 = AbstractC0053.m148(i2, "  ifields[", "]: ");
            sbM1482.append(this.instanceFields.get(i2));
            printWriterPrintWriterFor.println(sbM1482.toString());
        }
