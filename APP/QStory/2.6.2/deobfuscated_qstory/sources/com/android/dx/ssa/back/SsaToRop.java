...
public class SsaToRop {
    ...
    private BasicBlock convertBasicBlock(SsaBasicBlock ssaBasicBlock) {
        ...
        if (ropLabelSuccessorList.contains(exitBlock == null ? -1 : exitBlock.getRopLabel())) {
            if (ropLabelSuccessorList.size() > 1) {
                C2941.m6344(Hex.u2(ssaBasicBlock.getRopLabel()), "Exit predecessor must have no other successors");
                return null;
            }
    ...
    private BasicBlockList convertBasicBlocks() {
        ...
        C2632.m5294("Exit block must have no insns when leaving SSA form");
        return null;
    }
    ...
    private void verifyValidExitPredecessor(SsaBasicBlock ssaBasicBlock) {
        Rop opcode = ((SsaInsn) AbstractC6136.m11555(1, ssaBasicBlock.getInsns())).getOpcode();
        if (opcode.getBranchingness() == 2 || opcode == Rops.THROW) {
            return;
        }
        C2632.m5294("Exit predecessor must end in valid exit statement.");
    }
