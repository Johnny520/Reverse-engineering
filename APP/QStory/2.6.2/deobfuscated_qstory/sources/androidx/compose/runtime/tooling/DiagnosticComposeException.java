...
public final class DiagnosticComposeException extends RuntimeException {
    ...
    public DiagnosticComposeException(C1296 c1296) {
        ...
        while (i < size) {
            ...
            if (i4 < 0) {
                if (c1294.f3781 == 100) {
                    int i5 = i + 2;
                    if (i5 < size && ((C1294) list.get(i5)).f3781 == 1000) {
                        break;
        ...
        for (int i6 = 0; i6 < size2; i6++) {
            stackTraceElementArr[i6] = new StackTraceElement("$$compose", "m$" + ((C1294) arrayList.get(i6)).f3781, AttSourceFile.ATTRIBUTE_NAME, 1);
        }
        setStackTrace(stackTraceElementArr);
    }

    private final <T, R> R[] mapToArray(List<? extends T> list, InterfaceC6557 interfaceC6557) {
        list.size();
        AbstractC4394.m8919();
        throw null;
    }
    ...
    public String getMessage() {
        ...
        List list = c1296.f3784;
        list.getClass();
        C4327 c4327 = new C4327(list);
        int size = c4327.size();
        for (int i = 0; i < size; i++) {
            ((C1294) c4327.get(i)).getClass();
        }
        List listBuild = listBuilderM13685.build();
        listBuild.getClass();
        C4327 c43272 = new C4327(listBuild);
        int size2 = c43272.size();
        for (int i2 = 0; i2 < size2; i2++) {
            String str = (String) c43272.get(i2);
            sb.append("\tat ");
            sb.append(str);
            sb.append('\n');
        }
