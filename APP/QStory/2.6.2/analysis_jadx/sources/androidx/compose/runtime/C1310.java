package androidx.compose.runtime;

import java.util.Arrays;

/* JADX INFO: renamed from: androidx.compose.runtime.飘花落叶言子世哲楪苏兰, reason: contains not printable characters */
/* JADX INFO: compiled from: r8-map-id-cb39a6809a634dd4ad3d163e22b2e3b526599fa3253f8854b17de2b335a1a776 */
/* JADX INFO: loaded from: classes.dex */
public final class C1310 {

    /* JADX INFO: renamed from: 飘花落叶言子楪世苏兰哲, reason: contains not printable characters */
    public int f3801;

    /* JADX INFO: renamed from: 飘花落叶言子楪世苏哲兰, reason: contains not printable characters */
    public int[] f3802 = new int[10];

    /* JADX INFO: renamed from: 飘花落叶言子楪世哲苏兰, reason: contains not printable characters */
    public final void m2297(int i) {
        int[] iArrCopyOf = this.f3802;
        if (this.f3801 >= iArrCopyOf.length) {
            iArrCopyOf = Arrays.copyOf(iArrCopyOf, iArrCopyOf.length * 2);
            this.f3802 = iArrCopyOf;
        }
        int i2 = this.f3801;
        this.f3801 = i2 + 1;
        iArrCopyOf[i2] = i;
    }

    /* JADX INFO: renamed from: 飘花落叶言子楪世苏兰哲, reason: contains not printable characters */
    public final int m2298() {
        int[] iArr = this.f3802;
        int i = this.f3801 - 1;
        this.f3801 = i;
        return iArr[i];
    }

    /* JADX INFO: renamed from: 飘花落叶言子楪世苏哲兰, reason: contains not printable characters */
    public final int m2299(int i) {
        int i2 = this.f3801 - 1;
        return i2 >= 0 ? this.f3802[i2] : i;
    }
}
