...
class BSHType extends SimpleNode implements InterfaceC2656 {
    ...
    public Class<?> getType(CallStack callStack, Interpreter interpreter) throws EvalError {
        ...
        InterfaceC2618 typeNode = getTypeNode();
        ...
        if (!this.isListener) {
            interpreter.getClassManager().mo5090(this);
    ...
    public String getTypeDescriptor(CallStack callStack, Interpreter interpreter, String str) {
        ...
        InterfaceC2618 typeNode = getTypeNode();
        if (typeNode instanceof BSHPrimitiveType) {
            strM146 = getTypeDescriptor(((BSHPrimitiveType) typeNode).type);
        } else {
            ...
            if (cls != null) {
                strM146 = getTypeDescriptor(cls);
            } else if (str == null || Name.isCompound(strReplace)) {
                strM146 = "L" + strReplace.replace(TypePool.Default.LazyTypeDescription.GenericTypeToken.INNER_CLASS_PATH, '/') + ";";
            } else {
                StringBuilder sb = new StringBuilder("L");
                sb.append(str.replace(TypePool.Default.LazyTypeDescription.GenericTypeToken.INNER_CLASS_PATH, '/'));
                sb.append("/");
                strM146 = AbstractC0053.m146(sb, strReplace, ";");
            }
        }
        for (int i = 0; i < this.arrayDims; i++) {
            strM146 = AbstractC0053.m152("[", strM146);
        }
        this.descriptor = strM146;
        return strM146;
    }

    public InterfaceC2618 getTypeNode() {
