...
public final class LocalList extends FixedSizeList {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (entry.isStart()) {
                ...
                if (entryArr[register] == null) {
                    C2941.m6344(Integer.toHexString(entry.getAddress()), "redundant end at ");
                    return;
                }
                ...
                for (int i2 = i + 1; i2 < size; i2++) {
                    ...
                    if (entry3.getRegisterSpec().getReg() == register) {
                        if (!entry3.isStart()) {
                            C2941.m6344(Integer.toHexString(address), "redundant end at ");
                            return;
                        } else {
                            if (entry.getDisposition() != Disposition.END_REPLACED) {
                                C2941.m6344(Integer.toHexString(address), "improperly marked end at ");
                                return;
                            }
                            z = true;
                        }
                ...
                if (!z && entry.getDisposition() == Disposition.END_REPLACED) {
                    C2941.m6344(Integer.toHexString(address), "improper end replacement claim at ");
                    return;
                }
    ...
    public static class Entry implements Comparable<Entry> {
        ...
        public Entry(int i, Disposition disposition, RegisterSpec registerSpec) {
            if (i < 0) {
                C5919.m11249("address < 0");
                throw null;
            }
            if (disposition == null) {
                C2632.m5298("disposition == null");
                throw null;
            }
            try {
