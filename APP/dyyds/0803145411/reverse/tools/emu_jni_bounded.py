...
def hook_jni(uc, address, size, user_data):
    ...
    if name == 'RegisterNatives':
        ...
        uc.reg_write(UC_ARM64_REG_PC, x30); return
    if name == 'FindClass':
        uc.reg_write(UC_ARM64_REG_X0, 0x60000000)  # fake class
        uc.reg_write(UC_ARM64_REG_PC, x30); return
    if name in ('GetStringUTFChars','GetStringUTFRegion','GetStringRegion'):
        uc.reg_write(UC_ARM64_REG_X0, 0x60001000)
        uc.reg_write(UC_ARM64_REG_PC, x30); return
    if name in ('GetStringLength','GetStringUTFLength','GetArrayLength'):
        uc.reg_write(UC_ARM64_REG_X0, 0); uc.reg_write(UC_ARM64_REG_PC, x30); return
    # default: return 0
    uc.reg_write(UC_ARM64_REG_X0, 0)
...
# ---- run JNI_OnLoad bounded: capture ALL bss writes ----
...
# record every byte written to bss region 0x7b0000-0xce4000 with address
byte_writes = {}
def on_byte_write(uc, access, address, size, value, user_data):
    if 0x7b0000 <= address < 0xce4000 and size == 1:
        byte_writes[address] = value & 0xff
    elif 0x7b0000 <= address < 0xce4000 and size in (2,4,8):
        nbytes = size
        v = value
...
try:
    mu.emu_start(0x74e228, 0, count=150_000_000, timeout=0)
    print('JNI_OnLoad bounded done pc=%s' % hex(mu.reg_read(UC_ARM64_REG_PC)))
except UcError as e:
    print('bounded UcError:', e, 'pc=', hex(mu.reg_read(UC_ARM64_REG_PC)))
print('bss bytes written:', len(byte_writes))
# Reconstruct written regions into a buffer and find printable strings
if byte_writes:
    lo = min(byte_writes); hi = max(byte_writes)
    print('written range: 0x%x - 0x%x' % (lo, hi))
    buf = bytearray(hi - lo + 1)
    for a, v in byte_writes.items():
        buf[a - lo] = v
    # find runs of printable ASCII >= 6
    import re
    text = bytes(buf)
    for m in re.finditer(rb'[ -~]{6,}', text):
        s = m.group().decode('ascii','replace')
        print('  0x%x: %r' % (lo + m.start(), s))
    open(r'reverse/bss_all_writes.bin', 'wb').write(bytes(buf))
    print('saved bss_all_writes.bin (%d bytes)' % len(buf))
