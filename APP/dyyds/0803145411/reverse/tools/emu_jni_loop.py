...
# ---- run JNI_OnLoad from its start but stop after decrypt loop (0x74e3b4) ----
...
stop_after = [0x74e3b4]
def early_stop(uc, address, size, user_data):
    if address in stop_after:
        print('stopped at decrypt-loop end 0x%x' % address)
        uc.emu_stop()
mu.hook_add(UC_HOOK_CODE, early_stop)
try:
    mu.emu_start(0x74e228, 0, count=200_000_000, timeout=0)
    print('JNI_OnLoad(loop) done pc=%s' % hex(mu.reg_read(UC_ARM64_REG_PC)))
except UcError as e:
    print('loop UcError:', e, 'pc=', hex(mu.reg_read(UC_ARM64_REG_PC)))
try:
    dec = bytes(mu.mem_read(0xcdca80, 0x180))
    open(r'reverse/decrypted_loop.bin', 'wb').write(dec)
    print('decrypted area 0xcdca80:')
    pr = ''.join(chr(c) if 32<=c<127 else '.' for c in dec)
    for i in range(0, len(pr), 16):
        seg = dec[i:i+16]
        hexs = ' '.join('%02x'%b for b in seg)
        print('  +0x%03x: %-47s %s' % (i, hexs, pr[i:i+16]))
except UcError as e:
    print('dump failed:', e)
