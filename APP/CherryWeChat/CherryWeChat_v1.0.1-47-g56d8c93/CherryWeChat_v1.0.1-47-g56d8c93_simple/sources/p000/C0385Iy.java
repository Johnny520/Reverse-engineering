package p000;

import android.text.Editable;
import android.text.SpanWatcher;
import android.text.Spannable;
import android.text.TextWatcher;
import java.util.concurrent.atomic.AtomicInteger;

/* JADX INFO: renamed from: Iy */
/* JADX INFO: loaded from: classes.dex */
public final class C0385Iy implements TextWatcher, SpanWatcher {

    /* JADX INFO: renamed from: a */
    public final Object f1320a;

    /* JADX INFO: renamed from: b */
    public final AtomicInteger f1321b;

    public C0385Iy(Object r3) {
        this.f1321b = new AtomicInteger(0);
        this.f1320a = r3;
    }

    @Override // android.text.TextWatcher
    public final void afterTextChanged(Editable r2) {
        ((TextWatcher) this.f1320a).afterTextChanged(r2);
    }

    @Override // android.text.TextWatcher
    public final void beforeTextChanged(CharSequence r2, int r3, int r4, int r5) {
        ((TextWatcher) this.f1320a).beforeTextChanged(r2, r3, r4, r5);
    }

    @Override // android.text.SpanWatcher
    public final void onSpanAdded(Spannable r2, Object r3, int r4, int r5) {
        if (this.f1321b.get() > 0) goto L5;
    L7:
        ((SpanWatcher) this.f1320a).onSpanAdded(r2, r3, r4, r5);
        return;
    L5:
        if ((r3 instanceof C1086ZB) == false) goto L7;
    }

    @Override // android.text.SpanWatcher
    public final void onSpanChanged(Spannable r9, Object r10, int r11, int r12, int r13, int r14) {
        if (this.f1321b.get() > 0) goto L5;
    L7:
        ((SpanWatcher) this.f1320a).onSpanChanged(r9, r10, r11, r12, r13, r14);
        return;
    L5:
        if ((r10 instanceof C1086ZB) == false) goto L7;
    }

    @Override // android.text.SpanWatcher
    public final void onSpanRemoved(Spannable r2, Object r3, int r4, int r5) {
        if (this.f1321b.get() > 0) goto L5;
    L7:
        ((SpanWatcher) this.f1320a).onSpanRemoved(r2, r3, r4, r5);
        return;
    L5:
        if ((r3 instanceof C1086ZB) == false) goto L7;
    }

    @Override // android.text.TextWatcher
    public final void onTextChanged(CharSequence r2, int r3, int r4, int r5) {
        ((TextWatcher) this.f1320a).onTextChanged(r2, r3, r4, r5);
    }
}
