package io.github.cherrywechat.network.model;

import p000.AbstractC0295Gu;
import p000.AbstractC0628Oj;
import p000.C0809St;
import p000.C2521sz;
import p000.C2597up;
import p000.C2766ym;
import p000.InterfaceC0126Cx;
import p000.InterfaceC0406Jc;
import p000.InterfaceC0624Of;
import p000.InterfaceC0717Qm;
import p000.InterfaceC0919Va;
import p000.InterfaceC2677wj;

/* JADX INFO: loaded from: classes.dex */
public final /* synthetic */ class MusicAlbum$$serializer implements InterfaceC2677wj {
    public static final MusicAlbum$$serializer INSTANCE = null;
    private static final InterfaceC0126Cx descriptor = null;

    static {
        MusicAlbum$$serializer r0 = new MusicAlbum$$serializer();
        INSTANCE = r0;
        C0809St r1 = new C0809St("io.github.cherrywechat.network.model.MusicAlbum", r0, 3);
        r1.m1614l("id");
        r1.m1614l("name");
        r1.m1614l("picUrl");
        descriptor = r1;
    }

    private MusicAlbum$$serializer() {
    }

    @Override // p000.InterfaceC2677wj
    public final InterfaceC0717Qm[] childSerializers() {
        C2521sz r0 = C2521sz.f8819a;
        return new InterfaceC0717Qm[]{C2597up.f8972a, AbstractC0628Oj.m1249v(r0), AbstractC0628Oj.m1249v(r0)};
    }

    @Override // p000.InterfaceC0717Qm
    public final MusicAlbum deserialize(InterfaceC0406Jc r14) {
        InterfaceC0126Cx r0 = descriptor;
        InterfaceC0919Va r142 = r14.mo839B(r0);
        int r7 = 0;
        long r8 = 0;
        String r10 = null;
        String r11 = null;
        boolean r3 = true;
    L3:
        if (r3 == false) goto L16;
        int r4 = r142.mo1770j(r0);
        if (r4 != (-1)) goto L6;
        r3 = false;
        goto L3
    L6:
        if (r4 != 0) goto L7;
        r8 = r142.mo1767A(r0, 0);
        r7 = r7 | 1;
        goto L3
    L7:
        if (r4 != 1) goto L9;
        r10 = (String) r142.mo1769i(r0, 1, C2521sz.f8819a, r10);
        r7 = r7 | 2;
        goto L3
    L9:
        if (r4 != 2) goto L12;
        r11 = (String) r142.mo1769i(r0, 2, C2521sz.f8819a, r11);
        r7 = r7 | 4;
        goto L3
    L12:
        throw new C2766ym(r4);
    L16:
        r142.mo1777r(r0);
        return new MusicAlbum(r7, r8, r10, r11, null);
    }

    @Override // p000.InterfaceC0717Qm
    public final InterfaceC0126Cx getDescriptor() {
        return descriptor;
    }

    public final void serialize(InterfaceC0624Of r2, MusicAlbum r3) {
        InterfaceC0126Cx r0 = descriptor;
        r2.m1213c();
        MusicAlbum.write$Self$app_release(r3, null, r0);
        throw null;
    }

    @Override // p000.InterfaceC2677wj
    public InterfaceC0717Qm[] typeParametersSerializers() {
        return AbstractC0295Gu.f1004i;
    }

    @Override // p000.InterfaceC0717Qm
    public /* bridge */ /* synthetic */ Object deserialize(InterfaceC0406Jc r1) {
        return deserialize(r1);
    }

    @Override // p000.InterfaceC0717Qm
    public /* bridge */ /* synthetic */ void serialize(InterfaceC0624Of r1, Object r2) {
        serialize(r1, (MusicAlbum) r2);
    }
}
