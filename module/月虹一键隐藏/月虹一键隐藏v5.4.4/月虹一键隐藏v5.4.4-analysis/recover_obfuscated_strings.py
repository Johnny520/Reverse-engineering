...
class ConstantResolver:
    ...
    def _stack_store(self, before_index: int, base: str, displacement: int, depth: int) -> int | None:
        floor = max(0, before_index - 600)
        for index in range(before_index - 1, floor - 1, -1):
            ...
            if instruction.mnemonic in {"str", "stur"} and len(instruction.operands) >= 2:
                ...
                if location_base == base and int(location.mem.disp) == displacement:
                    return self.resolve(
    ...
    def resolve(self, register: str, before_index: int, depth: int = 0) -> int | None:
        ...
        for index in range(before_index - 1, floor - 1, -1):
            ...
            if mnemonic in {"ldr", "ldur"} and len(operands) >= 2:
                ...
                if base in {"sp", "x29"}:
                    return self._stack_store(index, base, displacement, depth + 1)
